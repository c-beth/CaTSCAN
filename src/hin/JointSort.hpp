#ifndef CATSCAN_JOINTSORT_HPP
#define CATSCAN_JOINTSORT_HPP


#include <iterator>

// The contents of this file were very directly adapted from https://bitbucket.org/seshadhri/escape/src/master/

namespace CaTSCAN {

    // Utility iterator for jointly sorting two arrays.
    // The first array is the key for the sort and the second is sorted with the first.
    ...
    struct JSIterator {
        ...
        struct reference {
            ...
            operator value_type() const { return {*key, *val}; }
        ...
        inline bool operator != (const JSIterator& other) const { return key != other.key; }
        ...
        JSIterator operator +(difference_type n) const { return {key + n, val + n}; }

        JSIterator operator -(difference_type n) const { return {key - n, val - n}; }

        difference_type operator -(const JSIterator& other) const { return key - other.key; }
